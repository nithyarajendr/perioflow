...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      <section>
        {/* Wrapper does the horizontal scroll on mobile so the table stays
            readable instead of being cut off. The right-edge fade is a
            visual hint that there's more to scroll to — fades out a bit on
            scroll-end via Tailwind's pseudo selector trick. */}
        <div className="relative bg-white border border-border-warm rounded-lg overflow-hidden">
          ...
          {/* Right-edge gradient hint that more columns are off-screen. md:hidden
              because on desktop the table fits and the fade would just look
              like a styling artifact. */}
          {sortedClaims.length > 0 && (
            <div
              aria-hidden="true"
              className="md:hidden pointer-events-none absolute top-0 right-0 bottom-0 w-8 bg-gradient-to-l from-white to-transparent"
            />
          )}
        </div>
      </section>
